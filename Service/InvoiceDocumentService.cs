...
namespace Bislerium.Service
{
    ...
    { 
        ...
        {
            var textStyle = TextStyle.Default.FontSize(15).SemiBold();
            ...
            container.Row(row =>
            {
                row.RelativeItem().Column(column =>
                {
                    ...
                    column.Item().PaddingTop(2).Text(text =>
                    {
                        ...
                        text.Span($"NRs. {totalAmount}").FontSize(14);
                    });
                });
            });
        }
        ...
        void ComposeTransactionsTableForSales(IContainer container)
        {
            container.Table(table =>
            {
                ...
                /*List<OrderItem> orderItemList = OrderItemServices.

                //Third process
                foreach (var order in orderObject.Orders)
                {
                    table.Cell().Element(CellStyle).Text((ReportObject.Orders.IndexOf(order) + 1).ToString());
                    table.Cell().Element(CellStyle).Text(order.CustomerName);
                    table.Cell().Element(CellStyle).Text(order.CustomerPhoneNumber);
                    table.Cell().Element(CellStyle).Text(order.Employee);
                    table.Cell().Element(CellStyle).Text($"Rs.{order.OrderTotalAmount}");
                    table.Cell().Element(CellStyle).Text($"Rs.{order.Discount}");
                    table.Cell().Element(CellStyle).Text($"Rs.{order.OrderTotalAmount - order.Discount}");
